"""sync_legistar.py — Sync Events, EventItems, and Votes from Legistar to Supabase.

Fetches events from the given --since date forward (including future scheduled
meetings already in Legistar), then pulls event_items and votes for those events.
...
def fetch_legistar(url: str, retries: int = 3) -> list:
...
def transform_event(r: dict) -> dict:
    return {
        ...
        "event_date":          r["EventDate"].split("T")[0] if r.get("EventDate") else None,
...
def main():
    parser = argparse.ArgumentParser(description="Sync Legistar events/items/votes to Supabase.")
    parser.add_argument(
        "--since",
        default=None,
        help="Start date YYYY-MM-DD (default: 14 days ago). Pulls all events from this date forward, "
             "including future scheduled meetings already in Legistar.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if not event_ids:
        print("No events found — nothing to sync for event_items or votes.")
        sys.exit(0)
    ...
    if not event_item_ids:
        print("No event items found — nothing to sync for votes.")
        sys.exit(0)
