...
**Fields we store in Airtable:** `EventId`, `EventBodyId` → Body (linked), `EventDate`, `EventTime`, `EventLocation`, `EventAgendaFile`, `EventMinutesFile`, `EventInSiteURL`, `EventVideoPath`, `EventAgendaStatusName` → EventAgendaStatus, `EventMinutesStatusName` → EventMinutesStatus, `EventLastModifiedUtc` → EventLastModified
